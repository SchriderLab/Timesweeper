...
def build_and_train_CNN(X_train, Y_train, X_valid, Y_valid, batch_sizes, lr, checkpoint_file_name, nClasses):
    ...
    c1 = Conv1D(128, kernel_size=2, 
                kernel_regularizer=keras.regularizers.l2(l2_lambda), 
                activation='relu')(input1)
    c2 = Conv1D(64, kernel_size=2, 
                kernel_regularizer=keras.regularizers.l2(l2_lambda),
                activation='relu')(c1)
    ...
    output = Dense(nClasses, kernel_initializer='normal',
                   activation='softmax')(dod)
...
def main():
    ...
    parser.add_argument(
        '-i', help="File with input data in NPZ format", dest='infile')
    parser.add_argument(
        '-c', help="Path/name of file in which the best network will be saved", dest='netfile')
    ...
    trainX, testX, valX = trainX.transpose(
        0, 2, 1), testX.transpose(0, 2, 1), valX.transpose(0, 2, 1)
