...
- [TimeSweeper](#timesweeper)
  ...
  - [Timesweeper Modules Details](#timesweeper-modules-details)
    - [Custom Simulation (`simulate_custom.py`)](#custom-simulation-simulate_custompy)
    - [stpopsim Simulation (`simulate_stdpopsim.py`)](#stpopsim-simulation-simulate_stdpopsimpy)
    - [Process VCF Files (`process_vcfs.py`)](#process-vcf-files-process_vcfspy)
    - [Make Training Data (`make_training_features.py`)](#make-training-data-make_training_featurespy)
    - [Neural Networks (`nets.py`)](#neural-networks-netspy)
    - [Timesweeper (`timesweeper.py`)](#timesweeper-timesweeperpy)
...
2. Simulate demographic model with time-series sampling
   1. `simulate_custom.py` if using custom SLiM script
   2. `simulate_stdpopsim.py` if using a SLiM script output by stdpopsim
...
### Custom Simulation (`simulate_custom.py`) 
...
### stpopsim Simulation (`simulate_stdpopsim.py`) 
...
### Process VCF Files (`process_vcfs.py`) 
...
### Make Training Data (`make_training_features.py`) 
...
### Timesweeper (`timesweeper.py`) 

Finally, the main module of the package is aptly named Timesweeper. This loads in the prepared VCF (see "Preparing Input Data for Timesweeper" below) in chunks, converts allele data to time-series allele velocity data, and predicts using the 1DCNN trained on simulated data. Each prediction represents a 51-SNP window with the focal allele being the actual target.
