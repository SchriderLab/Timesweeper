...
def create_hap_MS(srcDir: str, slimDir: str, baseLogDir: str) -> None:
    ...
    for mutdir in tqdm(
        glob.glob("{}/sims/*/muts/*".format(slimDir)),
        desc="\nSubmitting haplotype calculation jobs...\n",
    ):
        if "1Samp" not in mutdir:
            cmd = (
...
def format_haps(srcDir: str, slimDir: str, baseLogDir: str) -> None:
    """
    Runs SLURM jobs of of haps.py to format comboMS files generated by create_hap_ms() into trainable haplotype frequency spectrum feature files.
    ...
    Args:
        ...
        baseLogDir (str): Where to write logfiles from SLURM jobs to.
    """
    if not os.path.exists(os.path.join(baseLogDir, "hapformatlogs")):
        os.makedirs(os.path.join(baseLogDir, "hapformatlogs"))

    for repdir in tqdm(glob(slimDir + "/sims/*/muts/*")):
        if "1Samp" not in repdir:
            repID = repdir.split("/")[-1]
            repbasedir = "/".join(repdir.split("/")[:-2])
            if not os.path.exists(os.path.join(repbasedir, "haps")):
                os.makedirs(os.path.join(repbasedir, "haps"))

            cmd = "source activate blinx; python {}/src/haps.py {} {}".format(
                srcDir,
                repdir,
                repbasedir + "/haps/{}_hfs_data.npz".format(repID),
            )

            run_batch_job(
                cmd,
                "hapformat",
                "{}/jobfiles/hapformat.txt".format(slimDir, repID),
                "2:00:00",
                "general",
                "64GB",
                "{}/hapformatlogs/{}.log".format(baseLogDir, repID),
            )
...
def parse_arguments():
    ...
    if (
        args.run_func
        in ["launch", "clean", "make_feat_vecs", "make_haps", "format_haps"]
...
def main():
    ...
    if ua.run_func == "launch":
        ...
        format_haps(srcDir, slimDir, baseLogDir)
