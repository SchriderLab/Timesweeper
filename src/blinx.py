...
def format_haps(
    srcDir: str, slimDir: str, baseLogDir: str, spd: Dict, sweep_index: int
) -> None:
    """
    ...
    for repdir in glob(slimDir + "/sims/*"):
        if not os.path.exists(os.path.join(repdir, "haps")):
            os.makedirs(os.path.join(repdir, "haps"))
        ...
        run_batch_job(
            cmd,
            "hapformat",
            "{}/jobfiles/hapformat.txt".format(slimDir, repdir.split("/")[-1]),
            "1-00:00:00",
            "general",
            "2GB",
            "{}/hapformatlogs/{}.log".format(baseLogDir, repdir.split("/")[-1]),
        )
